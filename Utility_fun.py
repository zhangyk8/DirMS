...
'''
------------------
The following functions are coded for one-dimensional simulation study.
'''

## Truncate the Laplace distribution with center 0 on [-pi,pi] and then standardize it
def p2(x, scale=1.0):
    x_st = x%(2*np.pi)
    x_st[x_st > np.pi] = x_st[x_st > np.pi] - 2*np.pi
    return np.exp(-abs(x_st)/scale)/(2*scale*(1-np.exp(-np.pi/scale)))

### Function that sample data points from Density 2
def p2_samp(n, loc=0.0, scale = 1.0):
    data_ps = np.zeros((n,))
    can_sam = np.random.laplace(loc=loc, scale=scale, size=n)
    can_sam = can_sam[(can_sam >= -np.pi) & (can_sam <= np.pi)]
    cnt = len(can_sam)
    data_ps[:cnt] = can_sam
    while cnt < n:
        can_p = np.random.laplace(loc=loc, scale=scale, size=1)
        if can_p >= -np.pi and can_p <= np.pi:
            data_ps[cnt] = can_p
            cnt += 1
    return data_ps   ## (n,)-array

def f1(x, kappa=1.0, prob=[0.5, 0.5]):
    return prob[0]*p2(x) + prob[1]*np.exp(kappa*np.cos(x-np.pi/2))/(2*np.pi*sp.iv(0, kappa))
